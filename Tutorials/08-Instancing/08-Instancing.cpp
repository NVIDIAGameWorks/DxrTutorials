...
AccelerationStructureBuffers createTopLevelAS(ID3D12DevicePtr pDevice, ID3D12GraphicsCommandListPtr pCmdList, ID3D12ResourcePtr pBottomLevelAS, uint64_t& tlasSize)
{
    ...
    for (uint32_t i = 0; i < 3; i++)
    {
        ...
        instanceDescs[i].InstanceContributionToHitGroupIndex = 0;  // This is the offset inside the SBT. We only have a single geometry, so the offset 0
...
{
    // Compile the shader
    ID3DBlobPtr pRayGenShader = compileLibrary(L"Data/08-Shaders.hlsl", L"lib_6_1");
...
void DxrSample::createRtPipelineState()
{
    ...
    //  2 for miss and hit root-signature (signature and association)
    ...
    const WCHAR* missHitExportName[] = { kMissShader, hitProgram.exportName.c_str() };
...
void DxrSample::createShaderBindingTable()
{
    ...
    *(uint64_t*)(pData + progIdSize) = heapStart;
    ...
    // Entry 2 - hit program
    uint8_t* pHitEntry = pData + mSbtEntrySize * 2; // +2 skips the ray-gen and miss entries
...
void DxrSample::onLoad(SampleCallbacks* pSample, RenderContext::SharedPtr pRenderContext)
{
    ...
    createShaderResources(pSample->getWindow());    // Tutorial 06. Need to do this before initializing the SBT
...
void DxrSample::onFrameRender(SampleCallbacks* pSample, RenderContext::SharedPtr pRenderContext, Fbo::SharedPtr pTargetFbo)
{
    ...
    raytraceDesc.MissShaderTable.SizeInBytes = mSbtEntrySize;   // Only a s single miss-entry

    // Hit is the third entry in the SBT
    size_t hitSbtOffset = 2 * mSbtEntrySize;
    ...
    raytraceDesc.HitGroupTable.SizeInBytes = mSbtEntrySize;
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
    ...
    SampleConfig config;
    config.windowDesc.title = "Tutorial 08 - Instancing";
