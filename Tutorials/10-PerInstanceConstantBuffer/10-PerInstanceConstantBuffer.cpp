...
AccelerationStructureBuffers createTopLevelAS(ID3D12DevicePtr pDevice, ID3D12GraphicsCommandListPtr pCmdList, ID3D12ResourcePtr pBottomLevelAS, uint64_t& tlasSize)
{
    ...
    for (uint32_t i = 0; i < 3; i++)
    {
        ...
        instanceDescs[i].InstanceContributionToHitGroupIndex = i;  // This is the offset inside the SBT. Since we have unique constant-buffer for each instance, we need a different offset
...
{
    // Compile the shader
    ID3DBlobPtr pRayGenShader = compileLibrary(L"Data/10-Shaders.hlsl", L"lib_6_1");
...
void DxrSample::createShaderBindingTable()
{
    ...
    */
    ...
    uint32_t sbtSize = mSbtEntrySize * 5;
    ...
    *(uint64_t*)(pData + progIdSize) = heapStart;
    ...
    for (uint32_t i = 0; i < 3; i++)
    {
        uint8_t* pHitEntry = pData + mSbtEntrySize * (i + 2); // +2 skips the ray-gen and miss entries
...
void DxrSample::onFrameRender(SampleCallbacks* pSample, RenderContext::SharedPtr pRenderContext, Fbo::SharedPtr pTargetFbo)
{
    ...
    raytraceDesc.HitGroupTable.SizeInBytes = mSbtEntrySize * 3;
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
    ...
    SampleConfig config;
    config.windowDesc.title = "Tutorial 10 - Per-Instance Constant Buffer";
